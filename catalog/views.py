...
class BookCreate(CreateView):
    ...
    def form_valid(self, form):
        ...
        for gen in form.cleaned_data['genre']:
            #get the matching genre record from the database
            theGenre = get_object_or_404(Genre, name=gen)
...
def book_delete(request, pk):
    ...
    try:
        ...
        messages.success(request, (book.title + ' cannot be deleted. Book copies exist for this book'))
...
def loan_book_librarian(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # (set due date and update status of book)
